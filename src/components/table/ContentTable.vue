            ...
            <el-row class="ct-box__bottom">
              <el-col :span="6">
                  <el-popover
                    ...
                    trigger="hover"
                    >
                    <el-tag slot="reference" size="small" class="ct-box__tag ct-box__only">{{getPropText(item,status)|statusFilter}}</el-tag>
                ...
                <!--列表底部左侧状态-->
                  <el-tag v-else-if="item.cmsContent" size="small" class="ct-box__tag ct-box__only"
                    :class="{'ct-box__green': item.cmsContent && item.cmsContent.status === 10, 'ct-box__red': item.cmsContent && item.cmsContent.status === 11, 'ct-box__yello': item.cmsContent && item.cmsContent.status === 12 }">{{getPropText(item,status)|statusFilter}}</el-tag>
                    <span
                      v-show="iconIndex<4"
