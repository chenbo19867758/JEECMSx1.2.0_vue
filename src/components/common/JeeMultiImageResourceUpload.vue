  ...
  <section class="jee-multi-image-resource-upload-container">
    ...
    <div class="resource-upload-wrap">
      ...
      <div class="jee-hover-border" v-else>
        ...
        <el-button size="small" type="primary" @click="showResourceDialog" class='upload-image'>上传图片</el-button>
      </div>
      <draggable
        :list="fileList"
        v-bind="dragOptions"
          ...
          <div class="image-desc-wrap" :key="index">
            ...
            <div class="image-desc">
              <el-input
                ...
                :show-word-limit="isLengthLimit"
              ></el-input>
              <div class="image-desc-box" v-if="isBanText.indexOf(index) !== -1" v-html="banHtml(index)" @click="isBanShow(index)"></div>
            </div>
          </div>
        </template>
      </draggable>
  ...
  methods: {
    ...
    handleSubmit () {
      this.$refs.cropper.getCropBlob((data) => {
        let files = new File([data], this.fileList[this.fileIndex].fileName || 'cropper_image.png', { type: 'image/png', lastModified: Date.now() })
