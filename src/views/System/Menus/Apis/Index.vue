  ...
  <section class="apis-index-container">
    ...
    >
      <template #apiName="scope">
        <el-link :underline="false" :disable="!_checkPermission('/admin/ftp', 'GET')" class="jee-color" @click="rowHref(scope.scope.row)">{{scope.scope.row.apiName}}</el-link>
      </template>
    </base-table>
    <form-dialog
      ref="addDialog"
...
export default {
        ...
        handleColumn: [
          {
            ...
            name: '删除',
            icon: 'delete',
            disabled: !this._checkPermission('/admin/apis', 'DELETE')
          }
        ],
        handleColumnProp: {
          width: '75px'
        ...
        buttons: [
          {
            ...
            text: '新建',
            icon: 'xinjian',
            disabled: !this._checkPermission('/admin/apis', 'POST')
          },
          {
            type: 'Delete',
            text: '删除',
  ...
  methods: {
    ...
    handleConfirmAdd (data, btn) {
      ...
      this.$request.fetchApisCreate(data).then(res => {
        if (res.code === 200) {
