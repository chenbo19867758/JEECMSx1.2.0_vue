  ...
  <section class="security-index-container">
    ...
    >
      <template #ftpName="scope">
        <el-link :underline="false" :disable="!_checkPermission('/admin/ftp', 'GET')" class="jee-color" @click="rowHref(scope.scope.row)">{{scope.scope.row.ftpName}}</el-link>
      </template>
    </base-table>
    <form-dialog
      ref="addDialog"
...
export default {
        ...
        handleColumn: [
          {
            ...
            name: '删除',
            icon: 'delete',
            disabled: !this._checkPermission('/admin/ftp', 'DELETE')
          }
        ],
        handleColumnProp: {
          width: '75px'
        ...
        buttons: [
          {
            ...
            text: '新建',
            icon: 'xinjian',
            disabled: !this._checkPermission('/admin/ftp', 'POST')
          },
          {
            type: 'Delete',
            text: '删除',
  ...
  methods: {
    ...
    handleConfirmEdit (data) {
      ...
      this.editFormLoading = true
      this.$request.fetchFtpStorageUpdate(data).then(res => {
        if (res.code === 200) {
