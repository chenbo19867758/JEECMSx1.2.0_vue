  ...
  <section class="security-index-container">
    ...
    >
      <template #ossName="scope">
        <el-link :underline="false" :disable="!_checkPermission('/admin/ftp', 'GET')" class="jee-color" @click="rowHref(scope.scope.row)">{{scope.scope.row.ossName}}</el-link>
      </template>
    </base-table>
    <form-dialog
      ref="addDialog"
...
export default {
        ...
        handleColumn: [
          {
            ...
            name: '删除',
            icon: 'delete',
            disabled: !this._checkPermission('/admin/sysOss', 'DELETE')
          }
        ],
        handleColumnProp: {
          width: '75px'
        ...
        buttons: [
          {
            ...
            text: '新建',
            icon: 'xinjian',
            disabled: !this._checkPermission('/admin/sysOss', 'POST')
          },
          {
            type: 'Delete',
            text: '删除',
  ...
  methods: {
    ...
    handleConfirmEdit (data) {
      this.editFormLoading = true
      this.$request.fetchOssStorageUpdate(data).then(res => {
        if (res.code === 200) {
