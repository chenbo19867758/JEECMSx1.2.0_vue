  ...
  <section class="config-template-detail-container">
    ...
    <div>
      <el-button class='save-btn' size="small" type='primary' @click="handleConfirm"
        :disabled="!_checkPermission('/admin/template', news ? 'POST' : 'PUT')"
      >保存</el-button>
    </div>
  </section>
</template>
...
export default {
  ...
  methods: {
    ...
    upTemplate () {
      if (this.news) {
        this.$request.fetchTemplateCreate(this.form.form).then(res => {
          if (res.code === 200) {
        ...
        this.$request.fetchTemplateUpdate(this.form.form).then(res => {
          if (res.code === 200) {
            this.hideFormLoading()
            this.$message({
              message: '保存成功',
