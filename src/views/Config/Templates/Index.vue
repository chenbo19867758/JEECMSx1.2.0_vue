...
export default {
        ...
        handleColumn: [
          {
            ...
            name: '删除',
            icon: 'delete',
            disabled: !this._checkPermission('/admin/template', 'DELETE')
          }
        ],
        handleColumnProp: {
          width: '75px'
        ...
        buttons: [
          {
            ...
            text: '新建模板',
            icon: 'xinjian',
            disabled: !this._checkPermission('/admin/template', 'POST')
          },
          {
            type: 'Delete',
            text: '删除',
          ...
          },
          {
            ...
            text: '上传',
            icon: 'shangchuan',
            disabled: !this._checkPermission('/admin/template/upload', 'POST')
          },
          {
            type: 'New',
            text: '新建目录',
            icon: 'weidakaiwenjianjia',
            disabled: !this._checkPermission('/admin/template/dir', 'POST')
          }
        ],
        title: '',
        showAlertIcon: false
  ...
  methods: {
    ...
    fetchTemplateUniqueName (data) {
      ...
      this.$request.fetchTemplateUniqueName({ name }).then(res => {
        if (res.code === 200) {
          if (res.data) {
    ...
    beforeUpload (file, type) {
      ...
      if (this.list.params.root) {
        params.root = this.list.params.root
      }
      this.$request.fetchTemplateUniqueDirName(params).then(res => {
        if (res.code === 200) {
          if (res.data) {
    ...
    handleConfirmAddUpload (data, btn) {
      const param = new FormData() // 创建form对象
      param.append('uploadFile', data.file.raw, data.fileName)
      if (this.list.params.root) {
        param.append('root', this.list.params.root)
      }
    ...
    upTree (data) {
      if (data.directory) {
        this.list.params.root = data.name
        this.fetchTableApi()
      } else {
    ...
    handleConfirmAdd (data) {
      data.root = this.list.params.root
