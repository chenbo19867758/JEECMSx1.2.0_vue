...
export default {
  ...
  data () {
    ...
    const columns = [
      {
        ...
        scopeType: 'handle',
        type: 'Edit',
        disabled: !this._checkPermission('/admin/hotWords', 'PUT')
      },
      {
        prop: 'linkUrl',
        label: '链接',
    ...
    return {
      list: {
        ...
        params: {
          'hotWord': '',
          'hotWordCategoryId': Number(this.$route.query.id)
        },
        handleColumn: [
          {
            type: 'Delete',
        ...
        buttons: [
          {
            ...
            text: '新建',
            icon: 'xinjian',
            disabled: !this._checkPermission('/admin/hotWords', 'POST')
          },
          {
            type: 'Delete',
            text: '删除',
            icon: 'delete',
            disabled: !this._checkPermission('/admin/hotWords', 'DELETE')
          }
        ],
        title: '',
        showAlertIcon: true,
  ...
  watch: {
    '$route': function (newRoute, oldRoute) {
      if (newRoute.name === 'configVocabularyHotManage') {
        ...
        if (id) {
          this.list.params.hotWordCategoryId = Number(id)
          this.fetchTableApi()
        }
      }
    }
  ...
  methods: {
    ...
    handleConfirmAdd (data, btn) { // 新增用户
      ...
      var params = {}
      if (data.type === 'edit') {
        params = { 'id': data.id, 'hotWord': data.hotWord, 'linkUrl': data.linkUrl, 'isTargetBlank': data.isTargetBlank, 'remark': data.remark }
        ...
        params = {
          'hotWordCategoryId': data.hotWordCategoryId,
