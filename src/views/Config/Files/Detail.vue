...
export default {
    ...
    return {
      ...
      form: {
        ...
        submitBtns: [{
          text: '保存',
          subType: 'submit',
          disabled: !this._checkPermission('/admin/resource', this.news ? 'POST' : 'PUT')
        }]
      }
  ...
  methods: {
    ...
    fetchResourceCreate () {
      if (this.news) {
        this.$request.fetchResourceCreate(this.form.form).then(res => {
          if (res.code === 200) {
        ...
        this.$request.fetchResourcePut(this.form.form).then(res => {
          if (res.code === 200) {
            this.hideFormLoading()
            this.$message({
              message: '保存成功',
