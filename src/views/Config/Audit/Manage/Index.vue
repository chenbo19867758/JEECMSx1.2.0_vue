  ...
  <section class="audit-manage-index">
    ...
    <template #name="scope">
      ...
      <div class="nameBox" v-else>
        ...
        <div @click="editName(scope.scope.row)" :class="[{'event':!_checkPermission('/admin/auditStrategy/name', 'PUT')},'hide']">
          <jee-icon iconClass="bianji" :class="['icon', {'jee-popover':_checkPermission('/admin/auditStrategy/name', 'PUT')}]"></jee-icon>
        </div>
      </div>
    </template>
    <template #textAuditScene='scope'>
      <el-popover
        ...
        :visible-arrow='false'
        :class="{'event':!_checkPermission('/admin/auditStrategy/scene/text', 'PUT')}"
        trigger="click">
        <el-checkbox-group
          v-model="textChecked"
        ...
        <div slot="reference" :class="['jee-popover','fcs','jee-hover-color',{'jee-click':scope.scope.row.isText}]">
    ...
    <template #pictureAuditScene='scope'>
      <el-popover
        ...
        :visible-arrow= 'false'
        :class="{'event':!_checkPermission('/admin/auditStrategy/scene/picture', 'PUT')}"
        trigger="click">
        <el-checkbox-group
        ...
        <div slot="reference" :class="['jee-popover','fcs',{'jee-click':scope.scope.row.isImage}]">
  ...
  data () {
    ...
    const columns = [
      ...
      },
      {
        ...
        enableText: '开启',
        disableText: '关闭',
        disabled: !this._checkPermission('/admin/auditStrategy/open', 'GET')
      }
    ]
    return {
      temporaryText: [],
      ...
      textChecked: [],
      imageChecked: [],
      textList: ['色情识别', '暴恐识别', '政治敏感', '恶意推广', '低俗辱骂', '低质灌水'],
      imageList: ['暴恐违禁', '文本色情', '政治人物识别', '官方违禁库'],
      list: {
        ...
        handleColumn: [
          {
            ...
            name: '删除',
            icon: 'delete',
            disabled: !this._checkPermission('/admin/auditStrategy', 'DELETE')
          }
        ],
        handleColumnProp: {
          label: '操作',
        ...
        buttons: [
          {
            ...
            text: '创建策略',
            icon: 'xinjian',
            disabled: !this._checkPermission('/admin/auditStrategy', 'POST')
          },
          {
            type: 'Delete',
            text: '批量删除',
  ...
  methods: {
    ...
    textSort (objs) {
      objs.sort(function (a, b) {
        ...
        var order = ['色情识别', '暴恐识别', '政治敏感', '恶意推广', '低俗辱骂', '低质灌水']
    ...
    ImageSort (objs) {
      objs.sort(function (a, b) {
        // order是规则  objs是需要排序的数组
        var order = ['暴恐违禁', '文本色情', '政治人物识别', '官方违禁库']
        return order.indexOf(a) - order.indexOf(b)
      })
    },
    ...
    handleCheckedText (val, id) {
      let i = this.getArrDifference(this.temporaryText, val)
      console.log(this.temporaryText, val)
      console.log(i)
      if (i[0] === '色情识别') {
        i = 1
      } else if (i[0] === '暴恐识别') {
    ...
    handleCheckedImage (val, id) {
      let i = this.getArrDifference(this.temporaryImage, val)
      if (i[0] === '暴恐违禁') {
        i = 1
      } else if (i[0] === '文本色情') {
        i = 2
    ...
    handleConfirm (data) {
      ...
      this.checkedImage.forEach(v => {
        if (v === '暴恐违禁') {
          pictureScenes.push(1)
        } else if (v === '文本色情') {
          pictureScenes.push(2)
        } else if (v === '政治人物识别') {
          pictureScenes.push(3)
        } else if (v === '官方违禁库') {
          pictureScenes.push(4)
        }
      })
