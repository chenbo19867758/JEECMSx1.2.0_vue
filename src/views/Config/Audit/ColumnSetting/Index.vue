  ...
  data () {
    const columns = [
      ...
      },
      {
        ...
        disableText: '否',
        type: 'ForcedBy',
        disabled: !this._checkPermission('/admin/auditchannelset/pass', 'POST')
      },
      {
        prop: 'status',
        label: '状态',
        ...
        disableText: '关闭',
        type: 'Status',
        disabled: !this._checkPermission('/admin/auditchannelset/on', 'POST')
      }
    ]
    return {
      channelList: [],
        ...
        handleColumn: [
          {
            ...
            name: '删除',
            icon: 'delete',
            disabled: !this._checkPermission('/admin/auditchannelset', 'DELETE')
          },
          {
            type: 'AllDelete',
            name: '删除所有栏目',
      ...
      editButtons: [
        {
          text: '保存',
          type: 'Submit',
          disabled: !this._checkPermission('/admin/auditchannelset', 'POST')
        }
      ],
      editForm: {
        channelId: [],
  ...
  methods: {
    ...
    fetchSelectAuditchannelset (id) {
      this.$request.fetchSelectAuditchannelset({
        ...
        if (res.code === 200) {
          ...
          this.$refs.StrategyColumn.$refs.startTree[0].setCheckedKeys(this.ids)
        }
      })
    },
    // 保存选择栏目
    ...
    handleCheckAllChange () {
      if (this.editForm.checkAll === 1) {
        this.$refs.StrategyColumn.$refs.startTree[0].setCheckedNodes(this.editFormItems.find(v => v.prop === 'channelId').tree.filter(d => !d.gray))
        this.ids = this.$refs.StrategyColumn.$refs.startTree[0].getCheckedKeys()
      } else {
        this.$refs.StrategyColumn.$refs.startTree[0].setCheckedNodes([])
      }
    }
  },
  mounted () {
