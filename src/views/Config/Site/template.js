...
export default {
  ...
  watch: {
    // 'templatesForm.form.pcSolution': function (newData) {
    //   if (newData) {
    ...
    // },
    // 'templatesForm.form.mobileSolution': function (newData) {
    //   if (newData) {
    ...
    clearFormData (type) {
      Object.keys(this.templatesForm.form).forEach(k => {
        if (k.startsWith(`${type}Columns`) || k.startsWith(`${type}Contents`)) {
          delete this.templatesForm.form[k]
        }
      })
      if (type === 'pc') {
        this.templatesForm.form.pcHomePageTemplates = ''
      ...
      if (modelTpls instanceof Array && modelTpls.length) {
        modelTpls.forEach(d => {
          if (d.model && d.model.tplType === 1) {
            // this.$set(form, `pcColumns_${d.model.id}`, d.pcTplPath)
            // this.$set(form, `telColumns_${d.model.id}`, d.mobileTplPath)
        ...
        if (res.code === 200 && res.data instanceof Array) {
          ...
          columns.forEach(d => {
            if (d.options instanceof Array && d.options.length) {
              that.templatesForm.form[`pcColumns_${d.modelId}`] = that.templatesForm.form[`pcColumns_${d.modelId}`] === '' ? d.options[0].value : that.templatesForm.form[`pcColumns_${d.modelId}`]
            } else {
          ...
          contents.forEach(d => {
            if (d.options instanceof Array && d.options.length) {
              that.templatesForm.form[`pcContents_${d.modelId}`] = that.templatesForm.form[`pcContents_${d.modelId}`] === '' ? d.options[0].value : that.templatesForm.form[`pcContents_${d.modelId}`]
            } else {
        ...
        if (k.includes('_')) {
          ...
          if (model) {
            if (ids[0].startsWith('pc')) {
              model.pcTplPath = prop[k]
            } else {
              model.mobileTplPath = prop[k]
            }
          } else {
            let newModel = {
              modelId: ids[1]
            ...
            if (ids[0].startsWith('pc')) {
              newModel.pcTplPath = prop[k]
            } else {
              newModel.mobileTplPath = prop[k]
            }
            data.modelTpls.push(newModel)
          }
        }
      })
      this.$request.fetchSitesModelUpdate(data).then(res => {
        if (res.code === 200) {
          this.$message({
            message: '保存成功',
            type: 'success'
