  ...
  <jee-aside-layout class="config-site-detail-container">
    <section slot="asideHeader">
      <el-button
        ...
        :disabled="!_checkPermission('/admin/sites/recycle', 'GET')"
      >
  ...
  computed: {
        ...
        buttons: [
          {
            ...
            text: '新建子站点',
            icon: 'xinjian',
            disabled: !this._checkPermission('/admin/sites', 'POST') || !this.basicForm.form.newChildAble
          },
          {
            type: 'Preview',
            text: '预览',
            icon: 'yulang',
            disabled: !this.basicForm.form.viewAble
          },
          {
            type: 'Browse',
            text: '浏览',
          ...
          },
          {
            ...
            disabled: !this.basicForm.form.staticAble
          },
          {
            type: 'Permission',
            text: '权限分配',
            ...
            disabled: !this.basicForm.form.permAssignAble
          },
          {
            type: 'Shut',
            text: this.isOpen ? '关闭' : '开启',
            icon: 'jianqu',
            disabled: !this.basicForm.form.openCloseAble
          },
          {
            type: 'Delete',
            text: '删除',
            icon: 'delete',
            disabled: this.parentId === null || !this._checkPermission('/admin/sites', 'DELETE') || !this.basicForm.form.deleteAble
          }
        ],
        showAlertIcon: false
      }
    ...
    basicSubmitBtns () {
      return [{
        text: '保存',
        subType: 'submit',
        disabled: !this._checkPermission('/admin/sites', 'PUT') || !this.basicForm.form.editAble
      }]
    },
    getWorkOptions () {
    ...
    extSubmitBtns () {
      return [{
        text: '保存',
        subType: 'submit',
        disabled: !this._checkPermission('/admin/sites/ext', 'PUT') || !this.basicForm.form.editAble
      }]
    },
    templateSubmitBtns () {
      return [{
        text: '保存',
        subType: 'submit',
        disabled: !this._checkPermission('/admin/sites/model', 'PUT') || !this.basicForm.form.editAble
      }]
    }
  },
  methods: {
    ...mapActions('config'),
    ...mapActions('system', ['FetchSmsDetail', 'FetchEmailDetail']),
    // 预览
    handlePreview () {
      if (this.previewUrl) {
    ...
    handleConfirmBasic (data) {
      ...
      this.$request.fetchSitesUpdate(data).then(res => {
        if (res.code === 200) {
          this.$message({
            message: '保存成功',
            type: 'success'
    ...
    fetchSitesExtDetail () {
      ...
      this.$request.fetchSitesExtDetail({ id }).then(res => {
        if (res.code === 200) {
          ...
          this.expandForm.form.newContentResourceId = res.data.cmsSiteExt.newContentResourceId
    ...
    smsIsGloableHandler () {
      ...
      const that = this
      const props = ['smsTitle', 'serviceProviders', 'accesskeyId', 'accesskeySecret', 'messageSignatures']
      props.forEach(d => {
