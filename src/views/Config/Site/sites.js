...
export default {
          ...
          searchItems: [
            {
              ...
              icon: 'delete',
              text: '彻底删除',
              disabled: !this._checkPermission('/admin/sites/recycle', 'DELETE')
            },
            {
              type: 'henderBtn',
              btnType: 'Restore',
              icon: 'huanyuan',
              text: '还原',
              disabled: !this._checkPermission('/admin/sites/restore', 'POST')
            }
          ]
        },
        buttons: []
  ...
  methods: {
    ...
    handleConfirmAdd (data) {
      ...
      this.$request.fetchSitesCreate(data).then(res => {
        if (res.code === 200) {
          this.$message({
            message: '操作成功',
            type: 'success'
    ...
    recycleBinDelete (item) {
      ...
      this.msgDialog.content = '彻底删除后不可恢复，确认彻底删除选中的文件吗？'
