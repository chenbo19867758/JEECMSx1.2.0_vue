    ...
    <section class="map">
      ...
      <div style="display: flex">
        <div style="width:26%;padding-top: 40px;">
          <v-chart :forceFit="true" :height="cakeheight" :data="cakeData" :scale="cakescale">
        ...
        <div style="width:72%;margin-left:2%;padding-top: 40px;">
          <v-chart :forceFit="true" :height="tendencyHeight" :data="tendencyData">
            <v-tooltip />
...
export default {
  ...
  data () {
    return {
      headers: {
        ...
        title: '操作说明',
        showAlertIcon: true,
        content: '各类搜索引擎给您网站带来的流量情况。助您了解各个搜索引擎给您网站带来访客的情况，为合理优化搜索推广渠道提供数据支持。'
      },
      siteId: 1,
      params: {
        senior: false,
  ...
  methods: {
    ...
    handleSearchs () {
      ...
      this.fetchStatisticsSourceSearchEngine()
    },
    handleFocus () {
      this.params.time2 = ''
    },
    // 搜索引擎
    fetchStatisticsSourceSearchEngine () {
      this.params.siteId = this.siteId
      this.params.showType = this.tendencyRadio
      this.$request.fetchStatisticsSourceSearchEngine(this.params).then(res => {
        if (res.code === 200) {
          ...
          this.typeList.data = res.data.list
