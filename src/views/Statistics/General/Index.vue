    ...
    fetchStatisticsvisitSummarize () {
      ...
      this.$request.fetchStatisticsvisitSummarize(params).then(res => {
        if (res.code === 200) {
          ...
          this.visitorObj.oldRate = (this.visitorObj.oldUv / (this.visitorObj.newUv + this.visitorObj.oldUv)) * 10 + ''
          this.visitorObj.oldRate = this.visitorObj.oldRate.substring(0, this.visitorObj.oldRate.indexOf('.') + 4)
          this.visitorObj.oldRate = Number(this.visitorObj.oldRate)
        }
      })
    },
    // 来源网站
  ...
  watch: {
    ...
    tendencyRadio () {
      if (this.time.time === 0) {
        ...
        this.minTime = this.getTime(0)
        this.maxTime = this.getTime(6)
      } else if (this.time.time === 3) {
        this.minTime = this.getTime(0)
        this.maxTime = this.getTime(29)
      } else {
        this.minTime = null
        this.maxTime = null
      }
