<template>
  <div>
    <v-chart :forceFit="true" :height="height"
      :data="rateData" :scale="scale"
      :padding="[50, 50, 50, 50]"
    >
      <v-coord type="polar" :startAngle="-230" :endAngle="50" :radius="1"></v-coord>
      <v-axis
        dataKey="value"
        :zIndex="2"
        :line="null"
...
export default {
  ...
  computed: {
    ...
    rateData () {
      return [{ value: this.rate > 9.5 ? 9.5 : this.rate }]
    },
    arcGuide2End () {
      return [this.rate > 9.5 ? 9.5 : this.rate, 1.24]
    },
    htmlGuideHtml () {
      const rate = mul(this.rate, 10).toFixed(2)
      return '<div style="width: 150px;text-align: center;"><p style="font-size: 30px;color: #333333;margin: 0;line-height: 30px; padding-bottom:20px;">' + rate + '%</p><p style="font-size: 16px;color: #999999;margin: 0;padding-bottom:10px;">' + this.label + '</p></div>'
    }
  },
  props: {
    rate: {
