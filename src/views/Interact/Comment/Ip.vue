<template>
  <section class="interact-comment-ip-container">
...
export default {
  ...
  mounted () {
    this.fetchChannelTreeOption()
  },
  activated () {
    const { precisionIp } = this.$route.query
    if (precisionIp && precisionIp !== this.list.params.ip) {
      this.list.params.ip = precisionIp
      this.fetchCommentApi()
    }
  }
}
