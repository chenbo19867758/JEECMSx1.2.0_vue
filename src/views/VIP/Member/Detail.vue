    ...
    >
      ...
      <el-form-item class="form-item">
        <el-button  v-show="form.status !== '2'&& form.status !== '0'"
          ...
          size="small"
          @click="handleSubmit"
          :disabled="!_checkPermission('/admin/members', id ? 'POST' : 'PUT')"
...
import { desEncrypt, deepClone } from '@/utils'
        ...
        buttons: [
          {
            ...
            text: '审核通过',
            icon: 'shenghetongguo',
            disabled: !this._checkPermission('/admin/members/on', 'POST')
          },
          {
            type: 'AuditNo',
            text: '审核不通过',
            icon: 'shenghebutongguo',
            disabled: !this._checkPermission('/admin/members/off', 'POST')
          },
          {
            type: 'Deletes',
            text: '删除',
  ...
  methods: {
    ...
    handleSubmit (e, btn) {
      this.$refs.form.validate(valid => {
        if (valid) {
          this.loading = true
          this.$request.fetchMembersPut(this.getFormData).then(res => {
            if (res.code === 200) {
