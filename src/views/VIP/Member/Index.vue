...
export default {
  ...
  data () {
    const columns = [
      ...
      },
      {
        ...
        prop: 'enabled',
        label: '状态',
        disabled: !this._checkPermission('/admin/members/enable', 'POST') || !this._checkPermission('/admin/members/disable', 'POST'),
        minWidth: '80px'
      }
    ]
        ...
        handleColumn: [
          {
            ...
            name: '删除',
            icon: 'delete',
            disabled: !this._checkPermission('/admin/members', 'DELETE')
          }
        ],
        handleColumnProp: {
          width: '75px'
        ...
        buttons: [
          {
            ...
            text: '新建',
            icon: 'xinjian',
            disabled: !this._checkPermission('/admin/members', 'POST')
          },
          {
            type: 'Start',
            text: '启用',
            icon: 'qiyong',
            disabled: !this._checkPermission('/admin/members/enable', 'POST')
          },
          {
            type: 'Stop',
            text: '禁用',
            icon: 'jinyong',
            disabled: !this._checkPermission('/admin/members/disable', 'POST')
          },
          {
            type: 'Delete',
            text: '删除',
  ...
  methods: {
    ...
    handleConfirmAdd (data) {
      ...
      data.psw = desEncrypt(data.psw)
      data.json = {}
      data.json.input_cxQXwC = data.input_cxQXwC
      this.$request.fetchMembersAdd(data).then(res => {
        if (res.code === 200) {
