  ...
  data () {
    ...
    var footBtns = [
      ...
      },
      {
        ...
        power: 'MOVE',
        type: 'Move',
        disabled: !this._checkPermission('/admin/contentext/move', 'POST')
      },
      {
        name: '排序',
        type: 'Sort',
      ...
      },
      {
        ...
        power: 'DELETE',
        type: 'Delete',
        disabled: !this._checkPermission('/admin/contentext/rubbish', 'POST')
      },
      {
        name: '归档',
        type: 'File',
  ...
  methods: {
    ...
    setHeaderBth (data) {
      ...
      if (data.quote) {
        ...
        this.popverBtns.find(d => d.power === 'QUOTE').disabled = !this._checkPermission('/admin/contentext/quote', 'POST')
      }
        ...
        if (res.data.content && res.data.content.length) {
          let content = res.data.content.map((d, i) => {
            ...
            if (d.quote) {
              ...
              if (d.createType === 7) {
                obj = {
                  label: '镜像型引用，来源栏目：' + d.cmsContent.channelName,
                  icon: 'jingxiangyinyong'
                }
              } else if (d.createType === 6) {
                obj = {
                  label: '链接型引用，来源栏目：' + d.cmsContent.channelName,
