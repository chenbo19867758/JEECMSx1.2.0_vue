...
export default {
  ...
  methods: {
    ...
    handleEventAll (data, btn) {
      var callback = 'handleEvent' + btn.type
    ...
    handleCommand (ident) {
      ...
      if (ids.length > 0) {
        ...
        switch (key) {
          ...
          case 'type':
          // 类型
            this.handleEventCustom(ids, { id: value, addType: type })
            break
          default:
            break
        }
              ...
              if (arr instanceof Array) {
                arr.forEach((v, i) => {
                  arr2.push({
                    ...
                    disabled: !this._checkPermission('/admin/content', 'POST')
                  })
    ...
    // 改变状态
    changeStatus (data, status, message = '操作成功') {
      var ids = []
      if (data instanceof Array) {
        ids = data
      } else {
    ...
    handleEventSort (data, btn) {
      this.$refs.sortContent.showTheDialog(data, btn)
    },
