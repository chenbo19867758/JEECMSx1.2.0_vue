<template>
  <section class="handle-detail-hearder">
      <template v-for="(item, index) in handleHeaders">
        <div :key="index" class="header-button t-inline" v-if="!item.disShow&&item.showHandle">
          <div v-if="item.key === 'link'">
            <template v-for="(info, infoIndex) in item.options">
              <el-link
                ...
                :key="infoIndex"
                >
                <jee-icon :iconClass="info.icon"/>{{info.label}}
              </el-link>
            </template>
          </div>
          <el-popover
