...
export default {
  ...
  mutations: {
    ...
    // 设置是否全站开启短信服务
    SET_ISGLOABLE: (state, data) => {
      state.smsIsGloable = data.isGloable
      state.smsIsEnable = data.isEnable
    },
    // 设置是否全站使用邮件服务
  ...
  actions: {
    ...
    // 请求短信服务配置
    FetchSmsDetail ({ commit }) {
      request.fetchSmsDetail().then(res => {
        if (res.code === 200) {
          commit('SET_ISGLOABLE', res.data)
        }
      })
    },
    // 请求邮件服务配置
